...
function normalizeDraft(draft, payload) {
  ...
  const desiredOutcome = intake.desired_outcome || 'leads, sales, content, or automation';
  const sourceUrl = intake.website_or_social || 'No website or social link provided';
  ...
  const fallbackSections = {
    ...
    best_business_model: `Start with a service-led buildout instead of a complicated product. Package one clear offer, collect leads, qualify them, and use automation to keep follow-up consistent while the offer is refined.`,
    ...
    what_to_build_first: `Build the first lead capture system: landing page, intake form, CRM record, Discord owner alert, follow-up email/SMS path, and a manual review step before anything is sent externally.`,
    ...
    automation_stack_needed: `V1 stack: Vercel form, n8n webhook, Supabase records, Thurnos blueprint draft, Discord alert, and manual owner approval. Add payment, email delivery, and CRM tagging after the draft path is stable.`,
    ai_agents_needed: `Use Opportunity, Funnel, Revenue, Automation, and Execution agents under Thurnos. Their job is to turn the intake into a useful draft, not send promises directly to the lead.`,
    thirty_day_launch_roadmap: `Week 1: clarify offer and page copy. Week 2: wire intake, CRM, and alerts. Week 3: draft follow-up sequence and report. Week 4: test the full path with real conversations and refine based on conversion data.`,
    biggest_bottlenecks: `The main risks are unclear offer positioning, weak follow-up, no proof, and trying to build too many channels at once. Keep the first system narrow until there is traction.`,
...
const draft = normalizeDraft(extractJson(rawDraft), payload);
