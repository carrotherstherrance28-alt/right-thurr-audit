...
| --- | --- | --- | --- | --- |
| Vercel | Live / deployed | Production aliases `build.thurrsolutions.com` and `right-thurr-audit.vercel.app` are live. Production env includes Supabase, n8n webhook, Thurnos/OpenAI, Resend, and bridge secrets. `/api/buildout-request`, `/api/thurnos-blueprint`, `/api/approve-report`, `/api/review-reports`, and `/api/owner-access` exist. On 2026-05-06, the owner-auth default fix deployed and live unauthenticated `/api/owner-access` returns `Missing owner session` instead of preview mode; `/api/review-reports` also rejects unauthenticated requests. Diagnostic routes now serve `x-robots-tag: noindex, nofollow` after switching to Vercel `rewrites`. | Owner browser QA still needs signed-in Supabase session. | QA owner sign-in + report review queue gating in the browser, then run owner RLS hardening SQL. |
| GitHub Actions (CI) | Live | Workflow runs `npm ci` + `npm run ci` on pushes and PRs. `npm run ci` now runs `diagnostic:verify` + `build` + `diagnostic:verify:dist` so diagnostic lane invariants are enforced both pre-build and against the built `dist/diagnostic/*.html` artifacts. | None | Keep CI green; use `npm run ci` for local pre-push checks. |
...
| Video production kit | Website landlord asset + use-cases index + 25th use-case page + verification gate (noindex) | Phase-1 Branded AI Video Production Kit exists at `docs/project/Branded-AI-Video-Production-Kit.md` with reusable intake, motion/shot-plan, Runway prompt templates, and QA checklist under `docs/project/video-production-kit/`. Pilot #1 shot plan exists at `docs/project/video-production-kit/Pilot-Shot-Plan-Thurr-Solutions-Lead-System-Intro.md` (buyer-facing, no private data). Pilot #2 exists at `docs/project/video-production-kit/Pilot-Shot-Plan-Website-Landlord-Model.md`, based on the May 5, 2026 Granola note about the website landlord / rank-and-rent lead-flow model. Website landlord use-case asset draft (3 screenshot options + selected direction + 9:16 script + generation prompts + copyable page outline) lives at `docs/project/video-production-kit/Website-Landlord-Use-Case-Asset.md`, with SVG options at `design-options/website-landlord-usecase-options.html`. The repo-local use-cases lane now includes `/use-cases` + `/use-cases/website-landlord` + `/use-cases/ai-lead-followup` + `/use-cases/missed-call-textback` + `/use-cases/review-booster` + `/use-cases/no-show-saver` + `/use-cases/estimate-follow-up` + `/use-cases/seasonal-reactivation` + `/use-cases/quote-qualifier` + `/use-cases/service-reminder` + `/use-cases/maintenance-plan-upsell` + `/use-cases/deposit-request` + `/use-cases/financing-assist` + `/use-cases/referral-loop` + `/use-cases/parts-arrival-ping` + `/use-cases/eta-update` + `/use-cases/project-photo-proof` + `/use-cases/cancellation-save` + `/use-cases/invoice-follow-up` + `/use-cases/appointment-prep` + `/use-cases/after-hours-triage` + `/use-cases/service-recovery-save` + `/use-cases/warranty-claim-intake` + `/use-cases/change-order-approval` + `/use-cases/job-kickoff-packet` + `/use-cases/weather-delay-update` (kept `noindex`). Visual options for the latest page live at `design-options/weather-delay-update-usecase-options.html`. A repo-local guardrail verifies the `noindex` invariants (`npm run use-cases:verify`, wired into `npm run ci`). `npm run ci` passed locally on 2026-05-07. | None (repo-local). Live-account publishing is still blocked by user-owned logins/caps. | Draft the next use-case page (#26), or (when ready for public) remove `noindex` and add the approved route(s) to `public/sitemap.xml` after a compliance pass. |
...
```text
Primary: QA owner sign-in, run `docs/backend/Supabase-Owner-RLS-Hardening.sql`, and verify the private queue with a signed-in owner session on the live domain.
...
Extra fallback (repo-local, no logins): create the next use-case page (#26). Requirement: capture 3 visual options/screenshots before coding, then implement selected direction + run `npm run ci`.
```
