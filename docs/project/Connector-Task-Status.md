...
| Client diagnostic V1 | Visual options ready | Mobile detailing was selected as the first client diagnostic lane. V1 spec exists at `docs/product/Client-Diagnostic-V1-Mobile-Detailing.md`. Three screenshot options exist in `design-options/screenshots/`. | Needs a selected visual direction before frontend coding. AIDesigner MCP/API is not connected in this repo, so options were created locally from repo tokens. | Pick Option A, B, or C, then build the reusable diagnostic page template. |
...
| Marketplace sales | Launch kit ready | Upwork/Fiverr positioning, profile copy, gigs, buyer requirements, proposal template, and first-week action plan exist at `docs/sales/Marketplace-Launch-Kit.md`. | User must create/login to Upwork and Fiverr accounts and provide any required identity/tax/profile info. | Build profile assets/screenshots/PDF samples, then publish first Upwork/Fiverr offers. |
...
```text
Pick a mobile detailing diagnostic visual option, then build the reusable client diagnostic template. Retry Vercel production deploy after the daily cap resets.
```
